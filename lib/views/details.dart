...
class Details extends StatefulWidget {
  ...
  Details({Key key, this.entry, this.imgTag, this.titleTag, this.authorTag}) : super(key: key);
...
class _DetailsState extends State<Details> {
  ...
  void initState() {
    ...
    SchedulerBinding.instance.addPostFrameCallback(
      (_) {
        ...
        Provider.of<DetailsProvider>(context, listen: false).getFeed(widget.entry.author.uri.t.replaceAll(r'\&lang=en', ''));
      },
    );
  }
        ...
        return Scaffold(
          appBar: AppBar(
            actions: <Widget>[
              IconButton(
                ...
                icon: Icon(
                  detailsProvider.isFav ? Icons.favorite : Icons.favorite_border,
                  color: detailsProvider.isFav ? Colors.red : Theme.of(context).iconTheme.color,
                ),
              ),
            ],
          ),
          body: ListView(
            ...
            children: <Widget>[
              ...
              SizedBox(height: 30.0),
              sectionTitle('More from Author'),
              divider(),
              SizedBox(height: 10.0),
              moreBook(detailsProvider),
            ],
          ),
        );
      },
  ...
  imageTitle(DetailsProvider detailsProvider) {
    return Container(
      child: Row(
          ...
          Flexible(
            child: Column(
              ...
              children: <Widget>[
                ...
                SizedBox(height: 5.0),
                category(widget.entry, context),
                Center(
                  child: Container(
                    height: 20.0,
                    width: MediaQuery.of(context).size.width,
                    child: downloadButton(detailsProvider, context),
                  ),
                ),
              ],
            ),
  ...
  moreBook(DetailsProvider provider) {
    if (provider.loading) {
      return Container(
        height: 100.0,
        child: LoadingWidget(),
  ...
  downloadButton(DetailsProvider provider, BuildContext context) {
    return FlatButton(
      onPressed: () => launcher(widget.entry.link[0].href, 'Bookey'),
      child: Text(
        'Download',
      ),
    );
  ...
  void launcher(String url, String name) async {
    if (await canLaunch(url)) {
      await launch(url, forceWebView: true, enableJavaScript: true);
    }
  }

  category(Entry entry, BuildContext context) {
